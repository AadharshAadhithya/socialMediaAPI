...
def create_posts(post: schemas.PostCreate, db: Session = Depends(get_db), current_user=Depends(oauth2.get_current_user)):
    #idx =  cursor.execute("""INSERT INTO posts (title,content,published) VALUES (%s,%s,%s) RETURNING *""",(post.title,post.content,post.published))
    #idx = cursor.fetchone()
    # conn.commit()
...
def get_post(id: int, db: Session = Depends(get_db), current_user=Depends(oauth2.get_current_user)):
    #cursor.execute("""SELECT * FROM posts WHERE id= %s""",(str(idx),))
  ...
  #  if post.owner_id != current_user.id:
      #  raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
        # detail="Not authorized to perfom the action")
